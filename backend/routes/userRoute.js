...
router.patch('/following/:id', async(req, res) => {
    ...
    try {
        await User.findByIdAndUpdate(
            id, { $addToSet: { following: req.body.idToFollow } }, { new: true, upsert: true },
            (err, doc) => {
                if (err) {
...
router.patch('/unfollowing/:id', async(req, res) => {
    ...
    try {
        await User.findByIdAndUpdate(
            id, { $pull: { following: req.body.idToUnfollow } }, { new: true, upsert: true },
            (err, doc) => {
                if (err) {
                    return res.status(400).json(err);
                }
...
router.patch('/followers/:id', async(req, res) => {
    ...
    try {
        await User.findByIdAndUpdate(
            id, { $addToSet: { followers: req.body.followerId } }, { new: true, upsert: true },
            (err, doc) => {
                if (err) {
                    return res.status(400).json(err);
                }
...
router.patch('/unfollowers/:id', async(req, res) => {
    ...
    try {
        await User.findByIdAndUpdate(
            id, { $pull: { followers: req.body.unfollowerId } }, { new: true, upsert: true },
            (err, doc) => {
                if (err) {
                    return res.status(400).json(err);
                }
